...
## 十五、深度分析 v2 交付（2026-08-20，回应"研究太浅"的批评）

报告（同 URL 覆盖 v1）：https://claude.ai/code/artifact/73681ab0-6285-4230-bd43-b44fd73f0faa
复现材料：`docs/analysis/panel-study/`（pull.py 取数、analyze.py 分析、results.json、shocks_bj.tsv）

### 设计升级
- 队列级相关 → **个体级面板**：17,586 人 × 23 天 × 8 币 = 603,955 观测
- 相关系数 → **Treatment 框架**：每币自身 P80/P95 振幅分位定义中度/极端异动
- 单一 outcome → **看（主动）/买/被动曝光三分**；上下午拆分做时序
- 无特征 → **持仓规模/币种数异质性 + 个体 uplift 分布**

### 核心数字
- 极端异动日：看 2.15×、买 2.58×；剂量曲线 5% 振幅处拐点
- $10k+ 大户买 3.45×（0.64%→2.19%）；LINK/ETH 弹性是 XRP/DOGE 的 3-4 倍
- 个体：3.2% 高敏感（画像=持仓 2 倍+基线活跃 9.5%）vs 95.9% 无反应
- **8·19 时序**：价格爆发前的上午，主动看已 2.61×（惯性/曝光机械效应均排除）
  → 用户响应消息先于价格 → alert 应"价格∪消息"双通道触发

### 新踩的坑（都写进方法论）
1. **埋点表时间戳是 UTC+8**——用价格爆发小时 vs 下单峰值小时对齐验出；
   第一版 UTC 日线 treatment 错位 8 小时，重算后效应更强（错位在稀释信号）
2. **抽样必须验截断**：1/100 抽样恰好 50,000 行 = 撞 limit 被截尾（有偏），换 1/300 完整
3. **quoted heredoc 会把 `\$` 原样带进 SQL**——神策事件名 `$AppViewScreen` 经 shell 的
   转义链路极易坏，改用 python subprocess 传参（argv 不过 shell）
4. **老分区查不动**：0715/0701 的 COUNT 超 5 分钟被杀，0728 起可查；窗口受制于此
