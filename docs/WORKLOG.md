...
## 补丁（同日）：申请详情可查看 —— 用户一针见血指出的基础缺口

上线后 Lawrence 提交第一条真实申请（#5 营销感标签）即发现：**列表只有标题和状态，
审批人看不到申请的具体内容就要做决定**。`/detail` 接口在后端躺着，前端没接——
13 项机制级验证全绿，却没有一项在问"审批人能不能从头到尾完成审批"。

补：
- 列表条目整行可点展开详情（closest 委托，点标题/徽章都有效；键盘回车可达），
  按 kind 用中文标签铺开 payload 全部字段；已批准的单子附变更单（默认收起）
- 深链 `?lpane=3&cap=N` 直达并自动展开指定申请——通知可给直达链接
- 显隐用内联 style.display，不用 hidden 属性（当天刚踩过 display:flex 压过 [hidden] 的坑）
- 用真实 #5 验证 7 项：点子元素展开/收起、内容完整、点批准不误触、状态未被触碰；
  headless 截图像素级确认

经验落进长期记忆：`walk-each-role-journey-before-ship`——交付前按角色
（申请人/审批人/旁观者）把完整旅程在真实界面走一遍，只准用屏幕上有的东西。

---

# 需求 #100：第二次产出停摆（08-11 ~ 08-13，52.8h）—— VPN 断连

## 经过

用户报"线上服务挂了，我断了这台机器好几次"。诊断 2 分钟定位（靠 #98 加的端到端健康判据）：
站点 200、VM 已连续运行 18 天全绿——挂的是 **Mac 侧链路**：Mac 多次断电重启后
**GlobalProtect VPN 没有自动重连**，worker 每 5 分钟醒来但内网网关域名解析不了
（`nodename nor servname provided`），空转 52.8 小时，积压 16,637 条。

## 处置

- VPN 只能用户本人连（SSO/2FA），挂后台监视器（60s 一探，恢复即自动踢 worker + 催一轮 + 回报）
- 3 小时窗口到期未连 → 续 12 小时窗口 + 通知用户
- 用户连上后：网关首分钟撞 429 突发限流（重启突发打满 30/min 滚动窗），**下一分钟自愈**，
  成功率恢复 57/59；催一轮 pipeline，40 cache hits → 37 events，**首屏当即回到当天内容**
- 全程零数据丢失（staging 攒着，"新的先做"保首屏新鲜度，积压约 13h 追平）

## 与第一次停摆（#97）的对比与根治

两次停摆同构：**Mac 是生产链路的单点**，而它是台会被断电的工作机。
- #97：launchd 读不了 ~/Desktop（TCC）→ worker 从未跑起来
- #100：VPN 不自动重连 → worker 跑着但够不着网关

系统侧防线已尽（停摆横幅/健康判据/监视器/增量提交），但"连 VPN"这步只有用户凭据能做。
**根治建议（已提给用户）**：GlobalProtect 设 Connect at login / 让 IT 开 always-on。
更彻底的方案是把 enrich 算力挪到能直连内网的机器上，摆脱 Mac 单点——待用户决策。
