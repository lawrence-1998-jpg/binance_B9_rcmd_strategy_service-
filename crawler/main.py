...
# 分类，是要收的，不是要挡的）。
# 2026-07-29 补漏：原来只写了"上证综指/上证指数"这类**全称**，裸的"上证"漏网——
# 线上真出现过一条排到策略实验室第一名的「亚太股市重挫 上证失守3800」。财经标题
# 里指数名几乎总是简写，按全称匹配等于没匹配。改成匹配裸词。
#
# 这里有个刻意的取舍：像上面那条，报道主体其实是"亚太股市"，A 股只是其中一例，
# 按"只挡以 A 股为主体"的原意它算边界情况。仍然选择挡掉——标题里"上证失守3800"
# 是用户一眼就会读成 A 股内容的措辞，而同一场亚太下跌在库里有日经/KOSPI/恒生
# 的独立条目覆盖，挡掉不会丢失这个事件本身。宁可窄一点。
#
# "中证"是中证指数公司（A 股指数供应商），"沪深/深证/北证"同理，都不会误伤
# 中国宏观政策类内容——那类标题说的是货币政策/GDP/关税，不会出现交易所简称。
# `A股` 前面的 (?<![A-Za-z]) 不是洁癖，是修一个真的在库里造成误杀的 bug：
# 整条正则带 IGNORECASE，裸写 `A股` 会把**任何以字母 a 结尾的公司名 + 股**
# 全部判成 A 股——库里实测命中「Meta股价财报前跌近10%」「Cathie Wood买入Meta
# 股票」「GE Vernova股价目标」。这类误杀正好打在美股召回上，而美股偏少恰恰
# 是本期要解决的问题，属于自己给自己制造的问题。加上前瞻断言后，"Meta股"不
# 匹配，"A股""港股通A股"仍然匹配。
# 关键词表补到"板块/交易所/指数"三层。这张表在 2026-07-29 一天之内补了三次漏
# （先是裸"上证"、再是"Meta股"误杀、再是裸"创业板"），每次都是同一个错误：
# 写了指数全称漏了简称。所以除了把简称补全，更重要的是**不再把它当唯一防线**——
# pipeline 里加了一道基于 market_scope=cn_a_share 的语义闸（见 crawler/pipeline.py），
# 那道闸理解的是"这条报道的主体是不是中国大陆股市"，不依赖穷举措辞。
# 关键词表的定位退回它擅长的事：在花 LLM 钱之前先挡掉最明显的一批。
