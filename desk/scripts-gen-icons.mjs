/**
 * 生成 PWA 图标：墨色底 + 白色「拾」字。跟页面左上角那个标一致。
 *
...
const html = (size) => `<!doctype html><meta charset="utf-8"><style>
  ...
  div{width:${size}px;height:${size}px;background:#121a23;display:grid;place-items:center}
  ...
  i{position:absolute;width:${Math.round(size * 0.09)}px;height:${Math.round(size * 0.09)}px;border-radius:50%;
    background:#3346d3;left:${Math.round(size * 0.7)}px;top:${Math.round(size * 0.7)}px}
</style><div><span>拾</span></div><i></i>`
