...
const ctx = await b.newContext({ viewport:{width:430,height:932}, deviceScaleFactor:2, colorScheme:'dark', colorScheme:"dark" })
...
for (const [name, hash] of screens) {
  ...
  await pg.screenshot({ path: `${OUT}/d-${name}.png`, fullPage: true })
}
...
import('node:fs').then(fs => fs.writeFileSync(OUT + '/report-dark.json', JSON.stringify({ report, errs }, null, 1)))
...
for (const [k, v] of Object.entries(report)) {
  if (v.skipped) { t(`暗 ${k} 打得开`, false, v.skipped); continue }
  const c = (v.contrast||[]), tp = (v.touch||[])
  t(`暗 ${k} 对比度`, c.length === 0,
    c.slice(0,2).map(x=>`${x.ratio}:1 .${x.cls}「${x.text}」`).join('；'))
  t(`暗 ${k} 触控 ≥44`, tp.length === 0,
    tp.slice(0,2).map(x=>`${x.w}x${x.h} 「${x.label}」`).join('；'))
  t(`暗 ${k} 不横向溢出`, !v.overflow, v.overflow ? `${v.overflow.sw}>${v.overflow.iw}` : '')
}
t('暗 控制台没有报错', errs.length === 0, errs.slice(0,2).join(' | '))
await b.close()
